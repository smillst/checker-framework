...
def build_annotation_tools_release(version: str, afu_interm_dir: Path) -> None:
    ...
    gradle_cmd = (f"../gradlew copyWebsiteToDeployDir -Pdeploy-dir={afu_interm_dir}")
...
def build_checker_framework_release(
    ...
    # release_build.py from.
    gradle_cmd = f"./gradlew createCheckerFrameworkZip -Pchecker={checker_dir} -PdestDir={checker_framework_interm_dir}"
    execute(gradle_cmd, CHECKER_FRAMEWORK_RELEASE)
