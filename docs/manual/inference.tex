...
There are four scripts that you can use to run whole-program inference.
Each has advantages and disadvantages, discussed below:

\begin{itemize}
    \item
    To run whole-program inference on a single project without modifying its
    source code, use the \<wpi2.sh> script (Section~\ref{wpi-two}). This script
    requires you to make a few changes to your buildfile.
    ...
    However, it is based on a no-longer-maintained project, \<dljc>, so it is not guaranteed to work.
...
\sectionAndLabel{Running whole-program inference on a single project, with buildfile editing}{wpi-two}

You need to edit your buildfile, then run the \<wpi2.sh> script.


\subsectionAndLabel{Editing your buildfile}{wpi-two-buildfile}

First, set up your buildfile to run the Checker Framework.

Then, add the
\<-Aajava>,
\<-Ainfer>,
\<-AinferOutputDirectory>, and
\<-Awarns> command-line arguments.

Also remove or comment out \<-Werror>, because the
\<wpi2.sh> script will halt if the Checker Framework fails, including when it
issues an error.
...
checkerFramework {
  extraJavacArgs = [
    "-Ainfer=ajava",
    "-AinferOutputOriginal", // optional
    "-AinferOutputDirectory=" + project.projectDir + "/build/whole-program-inference",
    "-Aajava=" + project.projectDir + "/whole-program-inference-output",
    "-Awarns",
...
\subsectionAndLabel{Running \<wpi2.sh>}{wpi-two-command}

Second, run \<wpi2.sh>.  Its argument is the command that runs the Checker Framework.
Here is a typical invocation:

\begin{Verbatim}
  $CHECKERFRAMEWORK/checker/bin/wpi2.sh ./gradlew clean assemble
\end{Verbatim}

The \<clean> task is needed because \<wpi2.sh> runs the Checker Framework
repeatedly, and Gradle will skip compilation if do nothing if the old (but still
up-to-date) compilation output is present.

The output of \<wpi2.sh> is a top-level directory
\<whole-program-inference-output> that contains \<.ajava> files for each file of
your project.


\sectionAndLabel{Running whole-program inference on a single project, without buildfile editing}{wpi-one}
...
The results of invoking \<wpi.sh> are log files stored in the \<dljc-out/> folder
...
\item At least one of the \verb|JAVA_HOME|, \verb|JAVA17_HOME|,  or \verb|JAVA21_HOME|
...
(either because it contains a defect, or because it contains subtle code that
