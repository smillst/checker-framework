...
If the Java code permits every instantiation, then every qualifier is also
permitted.
Suppose a user declares a class as \code{class C<T> \ttlcb\ ...\ \ttrcb}
\textbf{or} as \code{class C<T extends Object> \ttlcb\ ...\ \ttrcb}.
Java allows any
instantiation of the class.  To also apply no restrictions,
the Checker Framework interprets the
declaration as \code{class C<T extends \emph{@TopType}
  Object> \ttlcb\ ...\ \ttrcb} rather than as \code{class C<T extends
  \emph{@DefaultType} Object> \ttlcb\ ...\ \ttrcb}.  For example, in the
Nullness type system (Section~\ref{nullness-qualifiers}), \code{class C<T>
\ttlcb\ ...\ \ttrcb} is interpreted as \code{class C<T extends @Nullable
  Object> \ttlcb\ ...\ \ttrcb} rather than as \code{class C<T extends
  @NonNull Object> \ttlcb\ ...\ \ttrcb}.  The latter would forbid
instantiations such as \code{C<@Nullable String>}, or would require
rewriting of code.

An \<Object> upper bound without a type qualifier in the relevant type
system --- for example, \code{class C<T extends Object>} --- is treated the
same as \code{class C<T>} because Java treats them the same.  In each case,
there is no restriction on the types a a client can use to instantiate the
class.

If a user writes an explicit bound such as \code{class C<T extends D>
  \ttlcb\ ...\ \ttrcb} where \<D> is not \<Object>, then the upper bound
\<D> is defaulted just like any other occurrence of \<D> in the program.

These rules means that the upper bound \<Object> in \code{class C<T extends
  Object>} is defaulted differently than the upper bound \<Date> in
\code{class C<T extends Date>}.  This is a bit unfortunate, but it is the
least bad option, as explained in Section~\ref{generics-defaults}.
...
unbounded wildcards or those bounded by \<Object>.  The two wildcards in the
following example are equivalent.

\begin{Verbatim}
class MyList<T> {}

MyList<? extends Date> listOfNonNulls;
MyList<@NonNull ? extends @NonNull Date> listOfNonNulls2;
\end{Verbatim}
