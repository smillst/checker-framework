...
declaration as \code{class C<T extends @Nullable
  Object> \ttlcb\ ...\ \ttrcb}.  (Treating it as \code{class C<T extends
  @NonNull Object> \ttlcb\ ...\ \ttrcb} would forbid
instantiations such as \code{C<@Nullable String>}, or would require
rewriting of code.)

On the other hand, if a user writes an explicit bound \emph{other than \<Object>},
such as \code{class C<T extends MyClass> \ttlcb\ ...\ \ttrcb}, then the upper bound
\<MyClass> is defaulted normally.  In the case of the Nullness Checker,
the declaration is treated as
\code{class C<T extends @NonNull MyClass> \ttlcb\ ...\ \ttrcb}.

% TODO: This paragraph is no longer correct, I think.
This rule means that the upper bound of \code{class C<T extends Object>} is defaulted
differently than the upper bound of \code{class C<T extends MyClass>}.
This is a bit unfortunate, but it is the least bad option, because it
ensures that \code{class C<T>} and \code{class C<T extends Object>}, which
are identical in Java, are treated identically by the Checker Framework.
