...
\\ \ref{faq-implicit-bounds}: How are implicit bounds defaulted?
...
\subsectionAndLabel{How are implicit bounds defaulted?}{faq-implicit-bounds}

The following two bits of code have the same semantics under Java; they
permit any instantiation:
...
\noindent
They are also treated the same by the Checker Framework: it permits any
instantiation, by giving the upper bound the top type qualifier (see
Section~\ref{climb-to-top}).

By contrast, a non-\<Object> upper bound without a type qualifier is
defaulted like any other ocurrence of the type.  That is, in

\begin{Verbatim}
class MyClass<T>                 ==  class MyClass<T extends @TOPTYPEANNO Object>
class MyClass<T extends Object>  ==  class MyClass<T extends @TOPTYPEANNO Object>
class MyClass<T extends MyClass> ==  class MyClass<T extendsf @DEFAULTANNO MyClass>
\end{Verbatim}

\noindent
\<Date> is defaulted like any other occurrence of \<Date>, which might not
be the top type qualifier.

This discrepancy is unfortunate, but is better than other alternatives; see
Section~\ref{generics-defaults}.
...
\item[\code{<T extends @NonNull Object>}]
  ...
  \code{T}.  Therefore, \code{<T extends @NonNull Object>} is equivalent to
...
\item[\code{<T extends @NonNull Object>}]
  ...
  It is equivalent to \code{<T extends @NonNull Object>}, as noted above.
