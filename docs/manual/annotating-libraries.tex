...
@AnnotatedFor() to each file you annotate.
...
warnings, because the JDK it not type-checked.
...
You can also concatenate all minimized \<.java> files into a single \<.astub> file:

\begin{Verbatim}
find project-stubs/ -name "*.java" -type f | xargs cat > project.astub
\end{Verbatim}

and then pass just that single file.
