...
example, \code{@Growable @MaybeShrinkable @MaybeReplaceable List} is a
subtype of \code{@MaybeGrowable @MaybeShrinkable @MaybeReplaceable List}
(because \<@Growable> $\le$ \<@MaybeGrowable>), but it is \emph{not} a
subtype of \code{@Ungrowable @MaybeShrinkable @MaybeReplaceable List}
...
subtype of \code{@Growable List} (Shrink defaults to \<@MaybeShrinkable> on the
right, Replace
defaults to \<@MaybeReplaceable> on both sides), but it is \emph{not} a
...
\begin{Verbatim}
  @Ungrowable @PolyShrinkable Set<K> keySet(@PolyShrinkable Map<K, V> map)
\end{Verbatim}

If the map is \code{@Growable @Shrinkable @Replaceable}, then
\code{keySet} returns
\code{@Ungrowable @Shrinkable @MaybeReplaceable Set}.

If the map is \code{@MaybeGrowable @MaybeShrinkable @MaybeReplaceable}, then
\code{keySet} returns \code{@Ungrowable @MaybeShrinkable @MaybeReplaceable Set}.

If the map is \code{@Ungrowable @Unshrinkable @Unreplaceable}, then
\code{keySet} returns \code{@Ungrowable @Unshrinkable @MaybeReplaceable Set}.
...
Queue> have \<@MaybeReplaceable> as their replaceability qualifier.  (In the
...
% LocalWords:  MaybeModifiable UnmodParam Growable MaybeGrowable MaybeShrinkable
