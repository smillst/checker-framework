...
called if they are supported, thus eliminating the possibility of an \UnsupportedOperationException  at runtime.
...
%% (Optional methods are unrelated to the \<Optional> type.) [[ I would just remove this sentence it is distracting.]]
...
A collection may support modifying a collection through one or more of the following capabilities.
...
\begin{itemize}
    \item \textbf{Grow}: The collection can grow by adding new elements
      (\<add>, \<addAll>, \<offer>, \ldots) at arbitrary locations. Not to be confused with sequential grow.
    \item \textbf{Sequential Grow}: A sequenced collection can grow by adding new
      ...
      \<putLast>). (This is shortened to SeqGrow in the rest of the manual.)
...
\<@Growable> &
  ordinary insertion: \<Collection.add>, \<addAll>, \<List.add>,
...
\<@SeqGrowable> &
  front/back insertion or positioning (only applicable to sequenced collections and maps):
...
\<@Shrinkable> &
  removal: \<Collection.remove>, \<removeAll>, \<retainAll>,
...
\<@Replaceable> &
  replacement without changing size: \<List.set>, \<List.replaceAll>,
...
When written on a non-void, one-argument method declaration, it indicates that the
...
If put on a void method or a method with no parameters, then it has no effect.
\end{description}
...
However, some static types lack methods for one or more mutation
...
rather than to the negative qualifier.  For example, \<Iterator> has
\<remove> but not \<add> or \<set>, so \<@Modifiable Iterator> carries
\<@Shrinkable>, but its grow and replace components are weakened to
\<@MaybeGrowable> and \<@MaybeReplaceable>.
...
\begin{tabular}{ll}
Capability & Weakening condition \\
\<Growable>/\<Ungrowable> &
  \<Map.Entry> or an \<Iterator> that is not a \<ListIterator> \\
\<SeqGrowable>/\<SeqUngrowable> &
  not a \<SequencedCollection>, \<SequencedMap>, or \<Deque> \\
\<Shrinkable>/\<Unshrinkable> &
  \<Map.Entry> \\
\<Replaceable>/\<Unreplaceable> &
  exact \<Collection>, any \<Set>, any non-\<LinkedList> \<Queue>,
  or an \<Iterator> that is not a \<ListIterator> \\
\end{tabular}
...
Using a top qualifier for a structurally unavailable capability is important.
For example, the static type \<Iterator> does not declare \<add> or \<set>, but
\<ListIterator> is a subtype of \<Iterator> and does declare those methods.
Therefore \<@Modifiable Iterator> should not mean that every possible runtime
iterator is incapable of grow or replace; it means only that those capabilities
are not known from the static type \<Iterator>.

Similarly, exact \<Queue> does not declare replacement methods, but a
\<Queue> reference can point to a \<LinkedList>, which does support
replacement operations.  This behavior ensures that code like this
type-checks:
...
% LocalWords:  PolyShrinkable PolyGrowable PolyReplaceable SeqGrow
