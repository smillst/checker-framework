...
\begin{verbatim}
  Grow hierarchy           Shrink hierarchy              Replace hierarchy             Iterator hierarchy
    ...
    @MaybeGrow             @MaybeShrink               @MaybeReplace          @MaybeIteratorPolyMod
      /      \                 /       \                    /       \                          |
...
example, \code{@Growable @MaybeShrink @MaybeReplace List} is a
subtype of \code{@MaybeGrow @MaybeShrink @MaybeReplace List}
(because \<@Growable> $\le$ \<@MaybeGrow>), but it is \emph{not} a
subtype of \code{@Ungrowable @MaybeShrink @MaybeReplace List}
...
Alias for \code{@MaybeGrow @MaybeShrink @MaybeReplace}.
...
\item[\refqualclass{checker/modifiability/qual}{UnmodParam}]
A syntactic sugar for \refqualclass{checker/modifiability/qual}{MaybeModifiable}.
It may only be written within a method or constructor parameter type, or within
an explicit receiver parameter type, when the method is not designed to change
the parameter.  It may appear nested within such a parameter type, such as in
\code{List<@UnmodParam List<String>>}.
...
\begin{Verbatim}
  ...
  void foo(@UnmodParam List<String> list) { ... }  // foo does not modify list
\end{Verbatim}
...
\begin{description}
\item[\refqualclass{checker/modifiability/qual}{MaybeGrow}]
...
\begin{description}
\item[\refqualclass{checker/modifiability/qual}{MaybeShrink}]
...
\begin{description}
\item[\refqualclass{checker/modifiability/qual}{MaybeReplace}]
...
\begin{Verbatim}
  @Growable @Shrinkable List<String>   // can grow and shrink, maybe replace
  @Growable @Replaceable List<String>  // can grow and replace, maybe shrink
...
subtype of \code{@Growable List} (Shrink defaults to \<@MaybeShrink> on the
right, Replace
defaults to \<@MaybeReplace> on both sides), but it is \emph{not} a
...
\code{keySet} returns
\code{@Ungrowable @Shrinkable @MaybeReplace Set}.

If the map is \code{@MaybeGrow @MaybeShrink @MaybeReplace}, then
\code{keySet} returns \code{@Ungrowable @MaybeShrink @MaybeReplace Set}.

If the map is \code{@Ungrowable @Unshrinkable @Unreplaceable}, then
\code{keySet} returns \code{@Ungrowable @Unshrinkable @MaybeReplace Set}.
...
Queue> have \<@MaybeReplace> as their replaceability qualifier.  (In the
...
class Demo {
  ...
  void combinedPermissions(@Growable @Replaceable Map<String, String> map) {
    ...
    map.remove("key");        // Error: @MaybeShrink (default) !<: @Shrinkable
  }
  ...
  void testPoly(@Modifiable List<String> mod, @Growable @Shrinkable List<String> gs) {
    ...
    // :: error: [assignment]
    @Modifiable List<String> bad = wrap(gs);  // Error: gs has @MaybeReplace
  }
}
\end{Verbatim}
...
% LocalWords:  MaybeModifiable UnmodParam Growable MaybeGrow MaybeShrink
% LocalWords:  MaybeReplace growable ListIterator PolyModifiable addFirst
