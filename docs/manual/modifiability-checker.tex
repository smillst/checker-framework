...
A collection can be modified in four different ways:
...
\begin{itemize}
    ...
    \item \textbf{SeqGrow}: A sequenced collection can grow by adding new
      ...
      \<offerFirst>, \<offerLast>, \<push>, \ldots).
...
The checker cannot determine whether this collection supports sequenced grow
operations.
...
It is not a subtype of \<SequencedCollection>, so it also has no
\<addFirst> or \<addLast> methods.  By contrast, \<SortedSet> is a
...
\<addLast> methods always throw \UnsupportedOperationException.
...
% LocalWords:  addLast offerFirst offerLast unreplaceable seq-growability
