...
An unfortunate consequence of the special case for \code{<T extends
  Object>} is that the ``\code{Object}'' in \code{MyClass<T extends
  Object>} is treated like local variable declarations which default to
top, but differently than every other occurrence of \code{Object} in the
program.  It is a bit confusing for different occurrences of \code{Object}
to mean different annotated types.
...
\item
  ...
  an the bottom annotation on the bound or on the wildcard itself.
