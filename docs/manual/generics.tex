...
\begin{Verbatim}
  ...
  class MyClass<T extends @NonNull Object> == class MyClass<T extends @NonNull Object>
\end{Verbatim}

The rationale for this choice is:
\begin{itemize}
\item
  The ``\code{<T>}'' in \code{MyClass<T>} and the ``\code{<T extends
  Object>} in ``\code{MyClass<T extends Object>}'' both mean
  ``fully unconstrained'' in Java.
  The rules make the type argument's qualifiers fully unconstrained, just
  as the Java basetype is.
\item
  The ``\code{Date}'' in \code{MyClass<T extends Date} is treated
  exactly like every other occurrence of \code{Date} in the program.  In
  particular, it is defaulted normally.
\end{itemize}
...
\item
  ...
  \code{class MyClass<T>} rather than \code{class MyClass<T extends
    ...
    Object>} rather than \code{class MyClass<T extends @NonNull Object>}.
\end{itemize}
