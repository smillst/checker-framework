...
\begin{Verbatim}
  class MyListOfNonNulls<T extends @NonNull Object> { ... }
  class MyListOfNullables<@Nullable T> { ... }
...
\item
If the \<extends> clause is \<extends Object>
(without an explicit type qualifier in the given type system),
then the upper bound also defaults to \<@\emph{TopType} Object>.
\item
If the \<extends> clause is written and contains no type qualifier and the
upper bound is not \<Object>,
...
(they permit every instantiation):
...
\noindent
and the upper-bound rules mean that a pluggable type system also
treats them identically, permitting every instantiation since the upper
bound is the top type qualifier.

The Nullness type system is an example.

\begin{Verbatim}
  class MyClass<T>                 ==  class MyClass<T extends @Nullable Object>
  class MyClass<T extends Object>  ==  class MyClass<T extends @Nullable Object>
  class MyClass<T extends Date>    ==  class MyClass<T extends @NonNull Date>
\end{Verbatim}

There are two unfortunate consequences of the special case for \code{<T
  extends Object>}.  First, \<T extends Object> and \code<T extends Date>
are treated differently.  Second, the ``\code{Object}'' in \code{MyClass<T
  extends Object>} is treated like local variable declarations and casts
which default to top, but differently than every other occurrence of
unannotated \code{Object} in the program.  It is a bit confusing for
different occurrences of unannotated \code{Object} to mean different
annotated types.

However, the rules maintain the equivalence of \code{<T>} and \code{<T
  extends Object>}:  both mean ``fully unconstrained''.
This enables treatment of bytecode (where it is impossibe to
distinguish the two forms) to be consistent with source code.
The rules require
less annotation of existing code that uses \code{<T extends Object>},
for type systems in which the top type is not the default type.
\label{default-is-not-top-type-systems}
These type systems include:
% This list is accurate as of 2023-03-09.  List is in alphabetical order.
\ahrefloc{fenum-checker}{Fake Enum}
  (default is \refqualclass{checker/fenum/qual},
  top is \refqualclass{checker/fenum/qual}{FenumTop})
\ahrefloc{guieffect-checker}{GUI Effect}
  (default is \refqualclass{checker/guieffect/qual}{AlwaysSafe},
  top is \refqualclass{checker/guieffect/qual}{UI}),
\ahrefloc{initialization-checker}{Initialization}
  (default is \refqualclass{checker/initialization/qual}{Initialized},
  top is \refqualclass{checker/initialization/qual}{UnknownInitialization}),
\ahrefloc{lock-checker}{Lock}
  (default is \<\refqualclass{checker/lock/qual}{GuardedBy}(\ttlcb\ttrcb)>,
  top is \refqualclass{checker/lock/qual}{GuardedByUnknown}),
\ahrefloc{must-call-checker}{Must Call}
  (default is \<\refqualclass{checker/mustcall/qual}{MustCall}(\ttlcb\ttrcb)>,
  top is \refqualclass{checker/mustcall/qual}{MustCallUnknown}),
\ahrefloc{nullness-checker}{Nullness}
  (default is \refqualclass{checker/nullness/qual}{NonNull},
  top is \refqualclass{checker/nullness/qual}{Nullable}),
\ahrefloc{signedness-checker}{Signedness}
  (default is \refqualclass{checker/signedness/qual}{Signed},
  top is \refqualclass{checker/signedness/qual}{UnknownSignedness}).

Here are some style guidelines:
\begin{itemize}
\item
  Omit the \<extends> clause when possible.  To indicate no constraints on
  a type qualifier, write \code{class MyClass<T>} rather than \code{class
    MyClass<T extends Object>}.
  This is shorter and is good Java style.
  For example, for the Nullness Checker, write
  \code{class MyClass<T>} rather than
  \code{class MyClass<T extends Object>}
  or
  \code{class MyClass<T extends @Nullable Object>}.
\item
  Use an \<Object> upper bound only when you need to write an explicit type
  annotation.  If you write just \<extends Object>, then someone who is
  reading the code might think that it is irrelevant (which it is in plain
  Java, and in type systems whose default type is their top type).  Also,
  some IDEs will suggest removing it.
\item
  When you write an \<extends> clause, you may need to write an explicit type
  annotation on it.
\item
  For upper-bounded wildcards (that is, \<?\ extends ...>), you can write
  the bottom annotation on the bound or on the wildcard itself.
  \<?\ extends @BottomType Object> is equivalent to \<@BottomType ?>, but
  the latter is shorter and may be easier to read.
\end{itemize}
...
\begin{Verbatim}
  ...
  class MyList3<T> { ... }
  class MyList3a<T extends @Nullable Object> { ... } // same as MyList3
\end{Verbatim}
