...
\refqualclass{framework/qual}{DefaultHasQualifierParameter} on a package is
equivalent to writing \<@HasQualifierParameter> on each class in that package
and all subpackages. The arguments to \<@DefaultHasQualifierParameter> are the
same as for \<@HasQualifierParameter>.
...
When using \<@DefaultHasQualifierParameter>, it's possible to disable
\<@HasQualifierParameter> for a specific class using
\refqualclass{framework/qual}{NoQualifierParameter}. Writing this on a class
indicates it has no implicit qualifier parameter and \<@HasQualifierParameter>
will not be enabled by default. Like \<@HasQualifierParameter>, it takes one or
more top annotations. It is illegal to explicitly write both
\<@HasQualifierParameter> and \<@NoQualifierParameter> on the same class for the
same hierarchy.
